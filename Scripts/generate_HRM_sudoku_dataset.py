...
def convert_subset(set_name: str, config: DataProcessConfig):
    ...
    with open(os.path.join(config.output_dir,f'{set_name}.csv'),'w') as f:
        for inp,lab in zip(results["inputs"],results["labels"]):
            inp = [str(int(a)) for a in inp.flatten()]
            lab = [str(int(a)) for a in lab.flatten()]
            f.write("".join(inp)+','+"".join(lab)+'\n')
