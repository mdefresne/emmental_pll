...
class CFN:
    def __init__(self, ubinit, resolution=6, vac=True, backtrack=10000, allSolutions=0):
        tb2.init()
        ...
        tb2.option.allSolutions = (
            allSolutions  # 000     # find all solutions up to a maximum limit
        )
        ...
        if len(iscope) == 0:
            ...
            for cost in costs:
                ...
                for r in range(len(iscope)):
                    i = len(iscope) - 1 - r
                    v = iscope[i]
                    if tuple[i] < self.CFN.wcsp.toValue(
                        v, self.CFN.wcsp.getDomainInitSize(v) - 1
                    ):
                        tuple[i] += 1
    ...
    def Solve(self):
        ...
        if solved:
            return (
                self.CFN.solution(),
                self.CFN.wcsp.getDPrimalBound(),
                len(self.CFN.solutions()),
            )
        else:
            return None
